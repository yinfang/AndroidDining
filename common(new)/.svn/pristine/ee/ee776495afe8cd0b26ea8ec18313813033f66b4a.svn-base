...
public class GlideUtil<T> {
    ...
    private int loadingImage = R.drawable.icon_empty;
     ...
     *              .load(T model)
     */
    protected void setImage(Context context, ImageView iv, T model, int resLoadingImage, int rounded, boolean
            isCircle) {
        ...
        BitmapRequestBuilder builder = getBitmapBuilder(context,  model, resLoadingImage);
     ...
     * crossFade() 一个淡入淡出动画方法还有另外重载方法 .crossFade(int duration)。如果你想要去减慢（或加快）动画，随时可以传一个毫秒的时间给这个方法。动画默认的持续时间是 300毫秒。
     */
    public BitmapRequestBuilder getBitmapBuilder(Context context,  T model, int resLoadingImage) {
        ...
        BitmapRequestBuilder Builder = request.asBitmap() //显示gif静态图片
                // .thumbnail(0.1f)//先加载缩略图 然后在加载全图
                .placeholder(resLoadingImage) // 占位符
    ...
    public void loadImage(Context context, ImageView imageView,
        ...
        BitmapRequestBuilder builder = getBitmapBuilder(context, model, loadingImage);
        if (isFade) {
            builder.priority(Priority.NORMAL) //下载的优先级
