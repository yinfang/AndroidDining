...
import com.clubank.util.ViewFactory;
...
/**
 * 实现可循环，可轮播的viewpager ,必须使用view_cycle_viewpager_contet ，view_cycle_viewpager_indicator.xml这两个布局，id必须一样
 */
@SuppressLint("NewApi")
public class CycleViewPager extends Fragment implements OnPageChangeListener {
    private static int indicatorDefult = R.drawable.icon_point;
    private static int indicator = R.drawable.icon_point_pre;
    private List<ImageView> imageViews = new ArrayList<ImageView>();
    private ImageView[] indicators;
    private FrameLayout viewPagerFragmentLayout;
    public LinearLayout indicatorLayout; // 指示器
    private BaseViewPager viewPager;
    private BaseViewPager parentViewPager;
    private ViewPagerAdapter adapter;
    private CycleViewPagerHandler handler;
    private int time = 2500; // 默认轮播时间
    ...
    private MyData infos;
    private String PicUrl = "PicUrl";//图片的url的key键，可通过setPicUrl方法设置前缀默认C.baseImageUrl请在项目中配置
    private String PicText="PicText";//图片对应左下角显示文字
    private  View view ;
    private MyData list;
    @Override
    ...
    public void setData(MyData list, ImageCycleViewListener listener) {
     ...
     */
    public void setData(MyData list, ImageCycleViewListener listener, int showPosition) {
        ...
        if (list.size() == 1) {
            isCycle=false;
            imageViews.add(ViewFactory.getImageView(getActivity(), C.baseImageUrl + list.get(list.size() - 1).getString(PicUrl)));
        ...
        if (list.size() > 1) {
            ...
            for (int i = 0; i < list.size(); i++) {
                imageViews.add(ViewFactory.getImageView(getActivity(), C.baseImageUrl +
                        list.get(i).getString(PicUrl)));
            }
            // 将第一个ImageView添加进来
            imageViews.add(ViewFactory.getImageView(getActivity(), C.baseImageUrl +
                    list.get(0).getString(PicUrl)));
        }
        ...
        TextView indicator_text=(TextView)view.findViewById(R.id.indicator_text);
        indicator_text.setText(list.get(0).getString(PicText));
    }
    ...
    final Runnable runnable = new Runnable() {
        ...
        public void run() {
            if (null != getActivity() && !getActivity().isFinishing()
                    && isWheel && null != infos && infos.size() > 1) {
    ...
    public void onPageScrollStateChanged(int arg0) {
        if (arg0 == 1) { // viewPager在滚动
           ...
           TextView indicator_text=(TextView)view.findViewById(R.id.indicator_text);
          indicator_text.setText(list.get(currentPosition-1).getString(PicText));
        }
        isScrolling = false;
    }
    ...
    private void setIndicator(int selectedPosition) {
        for (int i = 0; i < indicators.length; i++) {
            indicators[i]
                    .setImageResource(indicatorDefult);
        }
     ...
     * 设置图片路径属性名称默认PicUrl
     *
     * @param PicUrl 图片路径的属性名称
     */
    public void setPicUrl(String PicUrl) {
        this.PicUrl = PicUrl;
    }
    /**
     * 设置图片对应的描述文字属性名称默认PicTextl
     *
     * @param PicText 图片路径的属性名称
     */
     ...
     * 设置轮播图点点颜色
     *
     * @param indicatorDefult 默认灰色未选中的状态
     * @param indicator       选中的高亮状态
     */
    public void indicatorColor(int indicatorDefult, int indicator) {
        this.indicatorDefult = indicatorDefult;
