...
public class MyAsyncTask extends AsyncTask<Object, Result, Result> {
    ...
    private ACache mCache;//离线缓存
    private int isCache = 0;//是否缓存 0不缓存 1缓存
    private StringBuffer cacheName;//缓存文件名唯一标识(比如接口名称)，如果不同界面调用同一将接口参照：接口名称+不同类型type（没有就不用）+分页index
    ...
    public MyAsyncTask(Context context, Class<?> op, boolean showWaiting, int isCache, boolean updata) {
        this(context, op);
        this.updata = updata;
        this.isCache = isCache;
        if (isCache == 1) {
    ...
    public void run(Object... args) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (isCache == 1 && isCacheData) {//从缓存中取数据
                    ...
                    result.code = Integer.parseInt(mCache.getAsString(cacheName.toString() + "code"));
        ...
        if (isCache == 1 && !TextUtils.isEmpty(mCache.getAsString(cacheName.toString())) && !updata) {//如果有缓存并且undatafalse则不更新数据
            return;
        }
    ...
    public Result doInBackground(Object... args) {
        ...
        try {
            ...
            if (e instanceof UnknownHostException
                ...
                // || err != null
                // && (err.indexOf("START_TAG") > 0 || err.indexOf("END_DOC") > 0)
                    ) {
                result.code = RT.SOCKET_ERROR;
            ...
            if (isshowErrer) {
                ...
                if (result.code == RT.OPERATION_FAILED) {
                    if (result.obj != null) {
                        ...
                        UI.showToast(context, (String) result.obj,
                                Toast.LENGTH_SHORT);
                    }
                ...
                } else if (result.code == RT.CLUB_OFFLINE) {
                    result.msg = context.getString(R.string.club_offline);
                    UI.showToast(context, R.string.club_offline);
