 ...
 *
 */
public class UpdateRecommendImageTask extends ProgressTask {
	MyData data;
	BizDBHelper helper = new BizDBHelper(context, C.DB_NAME, null, C.DB_VERSION);
	DB db = null;

	public UpdateRecommendImageTask(Context context) {
		super(context);
		db = new DB(helper);
	}
	...
	protected void preWork() {
		data = db
				.getData("select code,imagedate from T_RecommendDish where extcode is not null order by code ");
	}
		...
		for (int i = 0; i < data.size(); i++) {
			try {
				...
				if (smallPicture != null && smallPicture.length() > 0) {
					...
					String sql = "update T_RecommendDish set smallpicture=?,largepicture=? ,imageDate=? where code=?";
